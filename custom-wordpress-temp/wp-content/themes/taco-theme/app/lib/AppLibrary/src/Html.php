...
class Html {
  ...
  public static function selecty($options, $selected=null, $attribs=array()) {
    if(array_key_exists('type', $attribs)) unset($attribs['type']);
    
    if(array_key_exists('options', $attribs)) unset($attribs['options']);
  ...
  public static function __callStatic($method, $args) {
    ...
    switch(count($args)) {
      case 0: return self::tag($element_type);
      case 1: return self::tag($element_type, $args[0]);
      case 2: return self::tag($element_type, $args[0], $args[1]);
