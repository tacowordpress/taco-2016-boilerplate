// https://code.google.com/p/jquery-debounce/source/browse/trunk/jquery.debounce.js?r=7
  $.extend({
...
;(function($){
  ...
  function check_images(container_selector) {
    ...
    if($eager.length) {
      $eager.filter(unloaded_image_selector).lazyLoad(ignore_range, true);
      $eager.filter(unloaded_bg_selector).lazyLoadBg(ignore_range, true);
      return false;
    }
    ...
    $lazy_bgs.each(function(){
      $(this).lazyLoadBg(ignore_range, true);
    });
  }
